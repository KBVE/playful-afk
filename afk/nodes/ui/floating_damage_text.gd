...
func _ready() -> void:
	# Set up label properties for damage text
	...
	# Set text color to bright red for damage
	add_theme_color_override("font_color", Color(1.0, 0.2, 0.2, 1.0))
...
## Initialize the damage text with amount and starting position
func setup(damage_amount: int, start_pos: Vector2) -> void:
	text = "-%d" % damage_amount
	position = start_pos
	elapsed = 0.0
	modulate.a = 1.0
	visible = true
